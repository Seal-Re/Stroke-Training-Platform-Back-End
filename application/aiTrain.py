...
from .utils import aiTips, questions_train_collection
...
def get_questions():
    ...
    try:
        ...
        system_prompt = f"""
你是一位专业的中国医生，需要根据以下分析结果 {ai_tips} 生成具有针对性的训练题目,帮助患者恢复。题目仅包含选择题和填空题，选择题的 model 为 "0"，填空题的 model 为 "1"。请按照以下 JSON 数组格式输出 20 道题目数据：
[
    {{
        "question": "题目内容",
        ...
        "answer": "答案",
        "score": 10,
        "class": "失算症训练|思维障碍训练|注意障碍训练|知觉障碍训练|记忆障碍训练",
        "model": "0|1"
    }},
    // 其他题目依此类推
]
请输出合法的 JSON 格式，确保符合上述结构，涵盖失算症训练、思维障碍训练、注意障碍训练、知觉障碍训练和记忆障碍训练五个方向，题目比例根据上述分析结果进行调整。在题目中避免使用图片。避免引入无关的概念，回答使用中文。
"""
        ...
        result = {
            "question_type": "training_questions",
            "questions": question_data
        }
