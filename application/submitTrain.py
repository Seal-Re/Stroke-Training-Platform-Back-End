...
import logging
from .utils import save_collection, deliver_score_train_collection, write_mongo_data
...
def submit_answers():
    ...
    try:
        ...
        if deliver_score_train_collection.find_one({"username": user}):
            deliver_score_train_collection.update_one({"username": user}, {"$set": deliver_score_data})
        else:
